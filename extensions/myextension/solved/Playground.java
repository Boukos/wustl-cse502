...
public class Playground implements Anim{
	...
	public int decidemonsterdir(){
		...
		*/
		...
		ShortestPaths sp = new ShortestPaths(graph, getvertex(monstercoord[0],monstercoord[1]), weights);
		sp.run();
		System.out.println(sp.returnValue(getvertex(pacmancoord[0],pacmancoord[1])));
		outputPath   = sp.returnPath(getvertex(pacmancoord[0],pacmancoord[1]));
		
		int spnext0 = outputPath.getFirst().to.coord[0] - monstercoord[0];
		int spnext1 = outputPath.getFirst().to.coord[1] - monstercoord[1];
		if(spnext0==1) return RIGHT;
		if(spnext0==-1) return LEFT;
		if(spnext1==1) return UP;
		if(spnext1==-1) return DOWN;
		return -1;
	}
