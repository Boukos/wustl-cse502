...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	void change(int loc1,int loc2){
		Decreaser<T>temp=this.array[loc1];
	...
	private void heapify(int parent) {
		...
		if(right<=size){
			if(array[left].getValue().compareTo(array[right].getValue())>=0){
				if(array[right].getValue().compareTo(array[parent].getValue())<0){
					change(parent,right);
					heapify(right);				
				}			
			}
			...
			{
				if(array[left].getValue().compareTo(array[parent].getValue())<0){
					change(parent,left);
					heapify(left);
				}
			}		
			...
			if(array[parent].getValue().compareTo(array[left].getValue())>0){
				change(left,parent);
				heapify(left);			
			}
		}				
